                    ...
                    <div class="col-span-12">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 space-y-1.5">
                                <label for="storyPhoto-{{ $i }}"
                                       class="file-pick-btn flex items-center justify-center gap-2 px-3 py-2 text-xs cursor-pointer">
                                    <svg wire:loading.remove wire:target="storyPhotos.{{ $i }}"
                                         class="w-3.5 h-3.5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5"/>
                                    </svg>
                                    <svg wire:loading wire:target="storyPhotos.{{ $i }}"
                                         class="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                                    </svg>
                                    <span wire:loading.remove wire:target="storyPhotos.{{ $i }}">
                                        {{ $previewUrl ? 'Ganti foto' : 'Pilih foto' }}
                                    </span>
                                    <span wire:loading wire:target="storyPhotos.{{ $i }}">Mengunggah...</span>
                                </label>
                                <input id="storyPhoto-{{ $i }}" type="file"
                                       wire:model="storyPhotos.{{ $i }}"
                                       accept="image/jpeg,image/png,image/webp"
                                       class="sr-only">
                                <p class="text-[10px] text-white/30">JPG / PNG / WebP &middot; maks 5 MB</p>
                                @if ($previewUrl)
                                    <button type="button"
